...
private function get_token($app_id,$app_key,$code,$callback,$state){
        if($state !== $_SESSION['state']){
           ...
           $url = "https://api.weibo.com/oauth2/access_token";
            $param = "grant_type=authorization_code&client_id={$app_id}&client_secret={$app_key}&code={$code}&state={$state}&redirect_uri={$callback}";
            ...
            //var_dump($params);
            return array('token' => $params->access_token,'openid' => $params->uid);
}
     ...
     * */
    public function get_user_info($app_id,$token,$uid){
