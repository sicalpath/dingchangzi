...
var DropDownList = (function($,undefined) {
    ...
    DDLClass.prototype = {
        ...
        _addListItem : function(opt){
            ...
            new_option.html('<i  class="dcz-icon icon-'+opt.value+'-sm"></i>'+opt.text);
        ...
        _createFromSelect : function() {
            ...
            if (this.select[0].selectedIndex>=0) {
                input.html('<i  class="dcz-icon icon-'+this.select[0].options[this.select[0].selectedIndex].value+'-md"></i><nobr>' + this.select[0].options[this.select[0].selectedIndex].text + '</nobr>');
            }
            icon.html('&#160');
            ...
            this.select.change(function(event) {
                if (self.select[0].selectedIndex>=0) {
                    input.html('<i  class="dcz-icon icon-'+self.select[0].options[self.select[0].selectedIndex].value+'-md"></i><nobr>' + self.select[0].options[self.select[0].selectedIndex].text + '</nobr>');
                }
            });
            // 改变选择时改变option图标
            this.input = input;
